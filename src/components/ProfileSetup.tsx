...
import { Camera, Sparkles, X, Loader2, ImagePlus, Search } from 'lucide-react';
...
interface ProfileSetupProps {
  ...
  onComplete: () => void;
  onGenderPreference?: (pref: string) => void;
}

export function ProfileSetup({ profile, onUpdate, onComplete, onGenderPreference }: ProfileSetupProps) {
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-background px-6 py-10">
      <motion.div className="w-full max-w-sm mx-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="mb-3 text-xs text-muted-foreground">Upload up to 5 clear photos • Max 5MB each</p>
        ...
        {uploadError && <p className="mb-3 text-xs text-destructive">{uploadError}</p>}
        ...
        <div className="mb-5 grid grid-cols-3 gap-2">
          {['Male', 'Female', 'Other'].map(g => (
            <button key={g} onClick={() => onUpdate({ ...profile, gender: g })}
              className={`rounded-xl border px-4 py-2.5 text-sm font-medium transition-all ${
                profile.gender === g ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-primary/50'
              }`}>
              {g}
            </button>
          ))}
        ...
        {/* Gender Preference - Who are you looking for */}
        <label className="mb-2 flex items-center gap-1.5 text-sm font-medium text-foreground">
          ...
          {GENDER_PREF_OPTIONS.map(g => (
            <button key={g} onClick={() => setGenderPref(g)}
              className={`rounded-xl border px-3 py-2.5 text-sm font-medium transition-all ${
                genderPref === g ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-primary/50'
              }`}>
              {g}
            </button>
          ))}
        ...
        <textarea placeholder="A little about you..." value={profile.bio || ''}
        ...
        <motion.button onClick={handleComplete} disabled={!isValid}
